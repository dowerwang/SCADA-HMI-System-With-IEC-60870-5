...
iec104_class::iec104_class()
{
    ...
    set.beginGroup("ProtocolConfiguration");

    Port=set.value("PORT INVERTER").toInt();
...
void iec104_class::clockSync()
{
    ...
    wapdu.asduh.oa = masterAddress;
    wapdu.asduh.ca = slaveAddress;
    wapdu.asdu103.ioa8 = 0;
    wapdu.asdu103.ioa16 = 0;
    wapdu.asdu103.time = obj.timetag;

    wapdu.asdu103.time.year = current.date().year() % 100;
    wapdu.asdu103.time.month = current.date().month();
    wapdu.asdu103.time.mday = current.date().day();
    wapdu.asdu103.time.hour = current.time().hour();
    wapdu.asdu103.time.min = current.time().minute();
    wapdu.asdu103.time.msec = current.time().second() * 1000 + current.time().msec();
    wapdu.asdu103.time.iv=0;
    wapdu.asdu103.time.su=0;
    wapdu.asdu103.time.wday=0;
    wapdu.asdu103.time.res1=0;
    wapdu.asdu103.time.res2=0;
    wapdu.asdu103.time.res3=0;
    wapdu.asdu103.time.res4=0;
